...
from FCSanalytical import FCSanalytical
from FCSanalytical import FCS2Canalytical
from plotPyCorrFit import plotPyCorrFit
from plotPyCorrFit import PyCorrFitData
from FCSanalytical import FCSDualFocus
import matplotlib.pyplot as plt


def FCSfit(Gexp, tau, fitfun, fitInfo, param, lBounds, uBounds, plotInfo, splitData=0, savefig=0):
    """
    ...
    plotFit(tau, Gexp, param, fitInfo, fitresult, plotInfo, savefig)
...
def FCSfitDualFocus(Gexp, tau, fitInfo, param, plotResults=True):
    """
    Fit experimental FCS data to the analytical model
    Assuming 3D diffusion in a Gaussian focal volume
    No triplet state assumed
    ...
    fitInfo     np.array boolean vector with
                    ...
                    1 if this value has to be fitted
                    0 if this value is fixed during the fit
    param       np.array vector with start values for all the pârameters
    ...
    fitresult = least_squares(fitfunDualFocus, fitparamStart, args=(fixedparam, fitInfo, tauall, Gall, splitData))
...
def fitfunDualFocus(fitparamStart, fixedparam, fitInfo, tau, yexp, splitData):
    """
    ...
    splitData       Vector with indices to split data
                        ...
                        E.g. [0 100 200] will split the data into twoo traces
...
def plotFit(tau, yexp, param, fitInfo, fitResult, plotInfo="", savefig=0):  
    # final parameters
    ...
    # plot
    plotPyCorrFit(data, plotInfo, savefig)
